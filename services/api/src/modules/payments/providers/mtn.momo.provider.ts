...
@Injectable()
export class MtnMomoProvider extends ProviderBase implements PaymentProvider {
  name: 'mtn' = 'mtn';

  constructor() {
    super(loadProviderConfig('mtn'));
  }
  ...
  async initiatePayment(input: InitiatePaymentInput, ctx: ProviderContext): Promise<InitiatePaymentResult> {
    ...
    const checkoutInstructions = String(
      res.data['checkoutInstructions'] || res.data['instruction'] || 'Approve payment on your MoMo prompt.'
    );
  ...
  webhookHandler(headers: Record<string, string | string[] | undefined>, payload: unknown): WebhookEvent {
    const signature = getHeader(headers, 'x-signature') || getHeader(headers, 'x-mtn-signature') || '';
