...
@Injectable()
export class AirtelTigoMoneyProvider extends ProviderBase implements PaymentProvider {
  name: 'airteltigo' = 'airteltigo';

  constructor() {
    super(loadProviderConfig('airteltigo'));
  }
  ...
  async initiatePayment(input: InitiatePaymentInput, ctx: ProviderContext): Promise<InitiatePaymentResult> {
    ...
    const checkoutInstructions = String(
      res.data['checkoutInstructions'] || res.data['instruction'] || 'Approve payment on your AirtelTigo prompt.'
    );
  ...
  webhookHandler(headers: Record<string, string | string[] | undefined>, payload: unknown): WebhookEvent {
    const signature = getHeader(headers, 'x-signature') || getHeader(headers, 'x-airteltigo-signature') || '';
